/* pg-ultra-shots */
(function(){
  if (window.pgUltraShots) return; window.pgUltraShots = 1;
  ...
  function seat(){
    ...
    if (!buy) return;
    var src = document.getElementById('pg-uls-shots-src');
    var shots = (src && src.querySelector('[data-uls="shots"]')) || buy.querySelector(':scope > [data-uls="shots"]');
    if (!shots) return;
  ...
  function loopRail(){
    var rail = document.querySelector('[data-uls-loop]');
    if (!rail) return;
    var sets = rail.querySelectorAll('.pg-uls-set');
  ...
  function loved(){
    var pill = document.querySelector('#pgx [data-uls-loved="pill"]');
